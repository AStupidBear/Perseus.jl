...
function distmat(D; g=0, s=0.2, N=5, C=2, plot=false, remove=true)
  ...
  path = normpath(joinpath(@__FILE__,"..","..","deps"))
...
function persdia(N, C, s)
  ...
  for b = 0:C-1
    ...
    for row = 1:size(A,1)
      A[row,2] == -1 && (A[row,2] = N)
      df = vcat(df, DataFrame(birth=s*A[row,1],death=s*A[row,2],BettiNumber="$b"))
    end
  end

  l1 = layer(df, x=:birth, y=:death, color=:BettiNumber, Geom.beeswarm)
  l2 = layer(x = [0,N*s], ymin = [0,0], ymax = [0,N*s], Geom.ribbon)
  p = plot(l1[1], l2[1], Theme(default_point_size=1.5pt)) |> display
end
